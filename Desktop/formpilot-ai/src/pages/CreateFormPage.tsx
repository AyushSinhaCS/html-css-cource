...
export default function CreateFormPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Uses the API Key defined in your Vite config and Render environment
      ...
      const result = await model.generateContent({
        contents: [{
          ...
          parts: [{
            ...
            Return ONLY a JSON object with "title" (string) and "questions" (array of objects).
            Each question object must have:
            - "type": (string, one of: "rating", "multiple_choice", "text")
            - "question": (string)
            - "options": (array of strings, required ONLY for multiple_choice)
            - "required": (boolean)
            
            RULES:
            1. rating and multiple_choice questions MUST be required: true.
            2. text questions MUST be required: false.`
          }]
        }],
        generationConfig: {
      ...
      alert("Failed to generate form. Please check your API Key in Render settings.");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#f7f7f5] text-[#111111] font-sans pb-24 relative">
      <DoodleBackground />
        ...
        {!generatedForm && !isGenerating && (
          <div className="bg-white rounded-xl border border-gray-200 p-10 mb-8 shadow-sm">
            ...
            <textarea
              ...
              className="w-full h-40 p-5 rounded-lg border border-gray-200 focus:border-black focus:ring-1 focus:ring-black outline-none resize-none text-lg bg-[#fcfcfc] transition-colors mb-6"
            />
            <button
              onClick={handleGenerate}
              disabled={!prompt.trim()}
              className="px-6 py-3 bg-black text-white rounded-md font-medium text-md hover:bg-gray-800 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed mb-10"
            >
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {examplePrompts.map((ex, i) => (
                  <button key={i} onClick={() => setPrompt(ex)} className="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-md text-sm hover:bg-gray-200 transition-colors">
                    {ex}
                  </button>
                ))}
              </div>
        ...
        {publishedUrl && (
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-10 mb-8 text-center animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center justify-center gap-2 max-w-lg mx-auto mb-8">
              ...
              <button onClick={copyToClipboard} className="p-3 bg-black text-white rounded-md hover:bg-gray-800 transition-colors">
                {copied ? <CheckCircle2 size={18} /> : <Copy size={18} />}
              </button>
            </div>
            <div className="flex justify-center gap-3">
              <a href={publishedUrl} target="_blank" rel="noreferrer" className="px-6 py-2.5 bg-white border border-gray-200 text-black rounded-md font-medium hover:bg-gray-50 transition-colors">
                Open Form
              </a>
            </div>
          </div>
            ...
            <div className="p-10 space-y-10">
              {generatedForm.questions.map((q, index) => (
                <div key={index} className="space-y-4">
                  ...
                  {q.type === "text" && <textarea disabled placeholder="Type your answer here..." className="w-full h-24 border-b border-gray-200 bg-transparent resize-none text-gray-500 text-lg" />}
                  {q.type === "rating" && <RatingInput value={0} onChange={() => {}} disabled />}
